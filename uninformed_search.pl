...
generate_step_grid(InitialGrid, Path, StepIndex, GeneratedGrid) :-
    ...
    ( SubPath = [] 
        -> GeneratedGrid = InitialGrid  % Handle empty path
...
% Main predicate to find the best path and print steps
...
% Print the initial grid, steps, and final grid
print_steps(Path) :-
    ...
    write('Final:'), nl, nl,
    generate_step_grid(InitialGrid, Path, PathLen, FinalGrid),  % Use full path
